...
from loss import L1_Loss, L1_Loss_transe
...
def parse_args():
    ...
    parser.add_argument("--lamda_", type=float, default=0.001)      # transe 正则项系数
...
def train(model, criterion, ctiterion_transe, optimizer, data, train_batch):
    model.train()
    x1 = model(data.x1, data.edge_index1, data.rel1, data.edge_index_all1, data.rel_all1)
    x2 = model(data.x2, data.edge_index2, data.rel2, data.edge_index_all2, data.rel_all2)
    loss = criterion(x1, x2, data.train_set, train_batch) + \
           ctiterion_transe.lamda_ * ctiterion_transe(x1, x2, data.edge_index1, data.rel1, data.edge_index2, data.rel2)
    optimizer.zero_grad()
...
def main(args):
    ...
    ctiterion_transe = L1_Loss_transe(args.lamda_)
    for epoch in range(args.epoch):
        if epoch%args.neg_epoch == 0:
            x1, x2 = get_emb(model, data)
            train_batch = get_train_batch(x1, x2, data.train_set, args.k)
        loss = train(model, criterion, ctiterion_transe, optimizer, data, train_batch)
